...
$(function () {
  ...
  $.getJSON('CAPBILITY_TOKEN_URL')
    .then(function (data) {
      ...
      // Setup Twilio.Device
      device = new Twilio.Device(data.token, {logLevel: 1, enableIceRestart: true});
      ...
      device.on('incoming', function (call) {
        log('Incoming connection from ' + call.parameters.From);
  ...
  onclick = function () {
    ...
    if (device) {
      const prom = device.connect(params);
      prom.then(call => {
        console.error('device.edge', device.edge);
        window.device = device;
        console.error('device.audio.inputDevice', device.audio.inputDevice);
        console.error('device.audio.outputDevice', device.audio.outputDevice);
        call.on('accept', function (conn) {
          console.error('Successfully established call!', conn);
        ...
        const listenFor = event => call.on(event, logMessage(event))
        listenFor('cancel');
